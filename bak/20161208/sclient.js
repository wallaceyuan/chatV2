...
var namBox = {root:origin,chatroom:chatroom,live:live,vod:vod,wechat:wechat,broadcast:broadcast,livetest:livetest};
                ...
                if(err){
                    ...
                    if(namBox[nsp]){
                        client.multi().lpush('messageKKDM'+nsp+room,JSON.stringify(result),redis.print).expire('messageKKDM'+nsp+room,600).exec(function (err, replies) {
                            if(err){
                                callback({"status":706,"msg":'sclient 154 ²éÑ¯´íÎó'},null);
                                console.log(8,err);
                            }else{
